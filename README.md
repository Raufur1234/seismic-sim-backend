...
It exposes two routes. `POST /compute` reruns the same modal/FFT
building-response solver (`mdof_response.py`) as the main project, against
...
- `code_design.py` — the code-design generator behind `/design`, an
  unmodified copy of the main project's file.
...
Whenever `server.py`'s `/compute` or `/design` logic, `mdof_response.py` or
`code_design.py` changes in
